...
export class TimeBlocksService {
  ...
  async findAll(userId: string, date?: string): Promise<TimeBlock[]> {
    ...
    return this.timeBlockModel.find(query).sort({ startTime: 1 }).exec();
  }

  async findById(id: string, userId: string): Promise<TimeBlock> {
    const timeBlock = await this.timeBlockModel.findById(id).exec();

    if (!timeBlock) {
      throw new NotFoundException(
        `Không tìm thấy khối thời gian với ID: ${id}`,
      );
    }
    ...
    if (timeBlock.user.toString() !== userId) {
      throw new ForbiddenException(
        'Bạn không có quyền truy cập khối thời gian này',
      );
    }
  ...
  async update(
    ...
    if (timeBlock.user.toString() !== userId) {
      throw new ForbiddenException(
        'Bạn không có quyền cập nhật khối thời gian này',
      );
    }
