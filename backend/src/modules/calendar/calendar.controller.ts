...
export class CalendarController {
  ...
  getEvents(@GetUser() user: UserDocument, @Query() query: CalendarQueryDto) {
  ...
  @ApiResponse({ status: 200, description: 'Thành công' })
  @ApiResponse({ status: 401, description: 'Chưa xác thực' })
  getDayData(@GetUser() user: UserDocument, @Param('date') date: string) {
  ...
  @ApiResponse({ status: 200, description: 'Thành công' })
  @ApiResponse({ status: 401, description: 'Chưa xác thực' })
  getWeekData(@GetUser() user: UserDocument, @Param('date') date: string) {
