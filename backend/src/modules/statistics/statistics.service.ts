...
export class StatisticsService {
  ...
  async getTasksStatistics(
    ...
    const end = endDate ? new Date(endDate) : new Date(); // Ngày hiện tại
    ...
    tasks.forEach((task) => {
      if (task.category) {
        ...
        const categoryName =
          (task.category as any).name || task.category.toString();
        byCategory[categoryName] = (byCategory[categoryName] || 0) + 1;
      } else {
        byCategory['Không có danh mục'] =
          (byCategory['Không có danh mục'] || 0) + 1;
      }
    });
  ...
  async getTimeBlocksStatistics(
    ...
    return {
      ...
      completionRate:
        totalHours > 0
          ? parseFloat(((completedHours / totalHours) * 100).toFixed(2))
          : 0,
      byDay,
    };
  }
  ...
  async getProductivityStatistics(
    ...
    const timeBlocksStats = await this.getTimeBlocksStatistics(
      userId,
      startDate,
      endDate,
    );
    ...
    const timeBlockCompletionRate =
      timeBlocksStats.totalHours > 0
    ...
    const productivityScore =
      taskCompletionRate * 0.6 + timeBlockCompletionRate * 0.4;

    // Tính điểm năng suất theo ngày
    const dailyScores = {};

    // Lấy tất cả các ngày từ cả hai thống kê
    const allDays = new Set([...Object.keys(timeBlocksStats.byDay)]);

    allDays.forEach((date) => {
      const timeBlockData = timeBlocksStats.byDay[date] || {
        total: 0,
        completed: 0,
      };
      const timeBlockScore =
        timeBlockData.total > 0
